        ...
        try {
            webSocket = client.newWebSocket(request, object : WebSocketListener() {
                override fun onOpen(socket: WebSocket, response: Response) {
                    if (webSocket !== socket) { socket.close(4002, "Stale connection"); return }
                    reconnectAttempt = 0; handler.removeCallbacks(reconnectRunnable); setConnectionState(STATUS_CONNECTED); updateNotification("Connected")
                    val deviceId = Settings.Secure.getString(contentResolver, Settings.Secure.ANDROID_ID) ?: "unknown-device"
                    val info = JSONObject().apply { put("manufacturer", Build.MANUFACTURER); put("model", Build.MODEL); put("androidApi", Build.VERSION.SDK_INT); put("appVersion", BuildConfig.VERSION_NAME) }
                    socket.send(JSONObject().apply { put("type", "register"); put("deviceId", deviceId); put("info", info) }.toString())
                }
                ...
                override fun onClosed(socket: WebSocket, code: Int, reason: String) { if (webSocket !== socket) return; webSocket = null; if (!stopping) scheduleReconnect() }
                override fun onFailure(socket: WebSocket, t: Throwable, response: Response?) { if (webSocket !== socket) return; webSocket = null; setConnectionState(STATUS_RECONNECTING); updateNotification("Connection lost; reconnecting"); if (!stopping) scheduleReconnect() }
            })
