...
class TostMediaWebSocket extends NativeTostWebSocket {
  constructor(...args) {
    ...
    this.addEventListener("message", event => {
      ...
      const detail = { id: message.id, result };
      const panel = mediaPending.get(message.id);
      if (panel) { mediaPending.delete(message.id); renderMediaFrame(panel, result); }
      else mediaFrames.set(message.id, result);
      window.dispatchEvent(new CustomEvent("tost-media-frame", { detail }));
    });
  }
}
...
async function mediaCommand(deviceId, command, panel) {
  ...
  try {
    ...
    if (mediaFrames.has(data.id)) { const frame = mediaFrames.get(data.id); mediaFrames.delete(data.id); mediaPending.delete(data.id); renderMediaFrame(panel, frame); }
    else panel.status.textContent = "Waiting for frame…";
    return data.id;
...
function scanMediaPanels() {
  for (const node of document.querySelectorAll(".device")) { const liveStatus = node.querySelector('[id^="live-status-"]'); if (liveStatus) addMediaPanel(node, liveStatus.id.slice("live-status-".length)); }
}
